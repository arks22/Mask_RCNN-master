...
#エラーを無視する？小松さんが書いていたものをとりあえず書いてみた
import warnings
...
class FilamentConfig(Config):
    ...
    #とりあえず2038ピクセルだからそこんとこ合わせるために追加しておいた
    IMAGE_MIN_DIM = 2048
    IMAGE_MAX_DIM = 2048
...
class FilamentDataset(utils.Dataset):
    def load_coco(self, dataset_dir, subset, year=DEFAULT_DATASET_YEAR, class_ids=None,
        ...
        #image_idsを作る
        #すべてのidがリスト状で入っているもの
        #自分の場合は１から数えているので別にリストで保存しなくても大丈夫そう
        #だからjpgの数がわかればOK

        #やはりimage_idsは必要っぽいので作っておきます
        image_ids = []

        jpg_path = "{}/{}_jpg".format(dataset_dir, subset)
        ...
        for i in range(0,len(jpg_files)):
            ...
            self.add_image(
                "filament", image_id=i,
                path=os.path.join(image_dir, jpg_files[i]),
                width=2048,
                height=2048,
    ...
    def image_reference(self, image_id):
        ...
        if info["source"] == "coco":
            ...
            super(FilamentDataset, self).image_reference(image_id)
